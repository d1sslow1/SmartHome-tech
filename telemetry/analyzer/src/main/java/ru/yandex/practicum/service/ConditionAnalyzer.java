...
public class ConditionAnalyzer {
    ...
    private boolean checkCondition(ScenarioCondition scenarioCondition, SensorsSnapshotAvro snapshot) {
        ...
        Object actualValue = extractValue(sensorState.getData(),
        ...
        log.debug("Condition result: sensor={}, actual={}, expected={}, op={}, result={}",
                scenarioCondition.getSensor().getId(), actualValue, expectedValue,
                scenarioCondition.getCondition().getOperation(), result);
    ...
    private Object extractValue(Object sensorData, ConditionType type) {
        return switch (type) {
            case MOTION -> sensorData instanceof MotionSensorAvro ?
                    ((MotionSensorAvro) sensorData).getMotion() : null;
            case LUMINOSITY -> sensorData instanceof LightSensorAvro ?
                    ((LightSensorAvro) sensorData).getLuminosity() : null;
            case SWITCH -> sensorData instanceof SwitchSensorAvro ?
                    ((SwitchSensorAvro) sensorData).getState() : null;
            case TEMPERATURE -> {
            ...
            case CO2LEVEL -> sensorData instanceof ClimateSensorAvro ?
                    ((ClimateSensorAvro) sensorData).getCo2Level() : null;
            case HUMIDITY -> sensorData instanceof ClimateSensorAvro ?
